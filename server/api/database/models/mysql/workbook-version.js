...
function generateUuidMapping(id) {
  var [type, sid] = id.split('-');
  var newUuid = `${type}-${md5(sid)}`;
...
async function generateWorkbookContent(workbookId, app) {
  debugger;
...
const WorkbookVersionWrapper = function(WorkbookVersion) {
  WorkbookVersion.observe('before save', async function(ctx) {
    ...
    if (is_create_mode) {
      ...
      var content = await generateWorkbookContent(instance.workbookId, app);
...
WorkbookVersionWrapper.generateWorkbookContent = generateWorkbookContent;

module.exports = WorkbookVersionWrapper;
