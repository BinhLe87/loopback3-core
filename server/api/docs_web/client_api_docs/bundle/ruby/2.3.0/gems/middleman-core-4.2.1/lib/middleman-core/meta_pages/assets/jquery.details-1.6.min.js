(function(b) {
  var d = 'open' in document.createElement('details'),
    e;
  ...
  e = function(a) {
    ...
    d ||
      (a
        .each(function() {
          var a = b(this);
          a.children('summary').length ||
            a.prepend('<summary>Details</summary>');
        })
        .children('summary')
        .filter(':not(tabindex)')
        .attr('tabindex', 0)
  ...
  b(function() {
    ...
    if (d)
      b('body').on('click', 'summary', function() {
        var a = b(this).parent();
        a.prop('open') ? a.trigger('close') : a.trigger('open');
      });
    else if (
      (b('html').addClass('no-details'),
      b('head').prepend(
        '<style>details{display:block}summary{cursor:pointer}details>summary::before{content:"\u25ba"}details.open>summary::before{content:"\u25bc"}details:not(.open)>:not(summary){display:none}</style>'
      ),
      b('body')
        .on('open.details', 'details', function(a) {
          b(this)
            .addClass('open')
            .trigger('change.details');
          a.stopPropagation();
        })
        ...
        .on('keyup', 'summary', function(a) {
          (32 === a.keyCode || (13 === a.keyCode && !b.browser.opera)) &&
            b(this)
              .parent()
              .trigger('toggle');
        }),
      b.browser.msie && 9 > b.browser.msie)
    )
      b('body')
