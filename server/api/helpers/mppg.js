/*
Ref: https://raw.githubusercontent.com/grantcarthew/node-mppg/master/mppg.js
Edited: Binhlv changed on April 2019: fix issue not return sibling value in func getNextSibligPath()
*/
// @ts-check
...
class MppgError extends Error {
  ...
  static get messages() {
    return Object.freeze({
      ...
      invalidPreviousSibling: 'Invalid previous sibling'
    });
  }
}
...
class MPPG {
  ...
  getNextSiblingPath(mpath) {
    ...
    //BinhLV edited on April 2019: add code in order to return sibling value
    let oneId = this.idLength * 1;
    mpath.length < oneId &&
      MppgError.throw(MppgError.messages.invalidPathLength);
    var cur_child_id = mpath.slice(-oneId);
    var next_sibling_child_id = this.getNextId(cur_child_id);
    return mpath.slice(0, -oneId) + next_sibling_child_id;
  }
