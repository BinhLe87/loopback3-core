...
Loggly.prototype.log = function (meta, callback) {
  ...
  //BinhLV: 10 June 2019
  //attempt replace `message` raw string by `message_json` json object if possible
  if (message.message_json && typeof message.message_json === 'object') {    
    delete message.message; //in order to avoid duplicate log info
  }
