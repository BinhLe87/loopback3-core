...
function build_run_docker() {
  ...
  exec_wrapper(
    `docker build ${docker_params.join(
      " "
    )} -t ${image_name}:${image_tag} -f Dockerfile.apc-master.${program.env} .`
  );

  exec_wrapper(
    `docker images | grep ${image_tag} | awk '{print $3}' | xargs -I {} docker run -it -d --name ${image_name}-${image_tag} -v /Users:/Users -p 49173:8080 -p 15672:15672 -p 5672:5672 -p 6379:6379 -p 5044:5044 {} tail -f /dev/null`
  );
