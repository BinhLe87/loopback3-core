...
const getAsync = (target_client=client) => promisify(target_client.get).bind(target_client);
const delAsync = (target_client=client) => promisify(target_client.del).bind(target_client);
 ...
 * @returns
 */
function create_redis_client(default_database_id = 0, should_use_client_cache = true, connection_options = {}) {
    ...
    client.select(default_database_id, (err, res) => {
        ...
        if (err) {
            logger.error(`Unable to select database id ${default_database_id} in Redis server at ${_get_redis_identity()}`);
            logger.error(err);
        } else {
    
            logger.info(`Successfully select database id ${default_database_id} in Redis server at ${_get_redis_identity()}`);
            logger.info(res);
...
function get_client_duplicate(should_use_client_duplicate_cache = true) {
...
async function get(key_name) {
    ...
    var target_client = client.pub_sub_mode > 0 ? get_client_duplicate() : client;
...
function subcribe_notify_keyspace_event(event_name) {
    ...
    if (!EVENT_NOTIFICATION_MAPPING[event_name]) {
        throw new Error(`Must pre-defined event_name ${event_name} in variable 'EVENT_NOTIFICATION_MAPPING'`);
    }

    client.once("ready", () => {

        client.config("SET", "notify-keyspace-events", EVENT_NOTIFICATION_MAPPING[event_name]);
...
client.on("error", function (err) {
    ...
    if (err) {
        logger.error(`Redis server has something wrong at ${_get_redis_identity()}`);
        logger.error(err);
    }
});
...
client.on("subscribe", function(channel, count) {
    ...
    logger.info(`Redis client start subscribing database ${selected_database_id} on channel ${inspect(channel)}`);    
});

client.on("message", async function(channel, key) {

    logger.info(`received message via subscribing channel ${inspect(channel)}:${key}`);
    ...
    if(callback && typeof callback == 'function') {
        ...
        logger.info(`Received 'reminder' type with key '${key_origin_without_prefix_reminder}' and value '${value}'`);
        callback_key_mapping[key_origin_without_prefix_reminder](key_origin_without_prefix_reminder, value);
    }
});
...
function validate_key_format(key) {
    ...
    throw new Error(`Redis key has invalid format. Must require format ${KEY_FORMAT_REGX.source}. You can call function generate_key() to generate correct key format`);
}
...
function generate_key(type, from_source, ...other_namespaces) {
    ...
    if (!type || !from_source) {
        ...
        throw new Error(`The function generate_key() requires at least first 2 arguments are 'type' and 'from_source'`);
    }
